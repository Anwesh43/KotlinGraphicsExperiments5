package com.example.lineshiftrotlineview
...
fun Canvas.drawLineShiftRotLine(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawLSRLNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = Color.parseColor(colors[i])
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineShiftRotLine(scale, w, h, paint)
}

class LineShiftRotLineView(ctx : Context) : View(ctx) {
    ...
    data class LineShiftRotLine(var i : Int = 0) {

        private var curr : LSRLNode = LSRLNode(0)
