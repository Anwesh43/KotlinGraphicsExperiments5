package com.example.linemultirotrightview
...
fun Canvas.drawLineMultiRotRight(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(3), h / 2) {
        ...
        drawXY(size + (w / 2) * (1 - dsc(2)), -size) {
            drawLine(0f, 0f, size, 0f, paint)
        }
    }
}

fun Canvas.drawLMRRNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = Color.parseColor(colors[i])
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineMultiRotRight(scale, w, h, paint)
}

class LineMultiRotRightView(ctx : Context) : View(ctx) {
    ...
    data class LineMultiRotRight(var i : Int) {

        private var curr : LMRRNode = LMRRNode(0)
