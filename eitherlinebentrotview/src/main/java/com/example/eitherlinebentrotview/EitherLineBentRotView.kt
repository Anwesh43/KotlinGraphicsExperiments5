...
class EitherLineBentRotView(ctx : Context) : View(ctx) {
    ...
    data class ELBRNode(var i : Int = 0, val state : State = State()) {

        private var next : ELBRNode? = null
        private var prev : ELBRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawELBRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : ELBRNode {
            var curr : ELBRNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class Renderer(var view : EitherLineBentRotView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val elbr : EitherLineBentRot = EitherLineBentRot(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            elbr.draw(canvas, paint)
            animator.animate {
                elbr.update {
                    animator.stop()
                }
