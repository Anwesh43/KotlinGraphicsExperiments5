...
class LineSweepRotArcView(ctx : Context) : View(ctx) {
    ...
    data class LSRANode(var i : Int = 0, val state : State = State()) {

        private var next : LSRANode? = null
        private var prev : LSRANode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLSRANode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LSRANode {
            var curr : LSRANode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineSweepRotArc(var i : Int) {

        private var curr : LSRANode = LSRANode(0)
    ...
    companion object {
        fun create(activity : Activity) : LineSweepRotArcView {
            val view : LineSweepRotArcView = LineSweepRotArcView(activity)
