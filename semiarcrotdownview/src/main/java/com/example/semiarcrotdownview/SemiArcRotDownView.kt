package com.example.semiarcrotdownview
...
fun Canvas.drawSemiArcRotDown(scale : Float, w : Float, h : Float, paint : Paint) {
    val size : Float = Math.min(w, h) / sizeFactor
    val dsc : (Int) -> Float = {
        scale.divideScale(it, parts)
    }
    drawXY((w / 2) * dsc(2), h * 0.5f * (1 + dsc(3))) {
        rotate(rot * dsc(1))
...
fun Canvas.drawSARDNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawSemiArcRotDown(scale, w, h, paint)
}

class SemiArcRotDownView(ctx : Context) : View(ctx) {
    ...
    data class SARDNode(var i : Int = 0, val state : State = State()) {
    ...
    data class SemiArcRotDown(var i : Int) {
    ...
    companion object {
        ...
        fun create(activity: Activity) : SemiArcRotDownView {
            val view : SemiArcRotDownView = SemiArcRotDownView(activity)
