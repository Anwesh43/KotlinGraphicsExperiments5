...
class BarHalfArcSweepLeftView(ctx : Context) : View(ctx) {
    ...
    data class BHASLNode(var i : Int = 0, val state : State = State()) {

        private var prev : BHASLNode? = null
        private var next : BHASLNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBHSLNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BHASLNode {
            var curr : BHASLNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BarHalfArcSweepLeft(var i : Int = 0) {

        private var curr : BHASLNode = BHASLNode(0)
