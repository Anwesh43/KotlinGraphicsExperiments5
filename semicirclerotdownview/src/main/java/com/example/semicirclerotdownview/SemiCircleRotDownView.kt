package com.example.semicirclerotdownview
...
fun Canvas.drawSemiCircleRotDown(scale : Float, w : Float, h : Float, paint : Paint) {
    val size : Float = Math.min(w, h) / sizeFactor
    val dsc : (Int) -> Float = {
        scale.divideScale(it, parts)
    }
    drawXY(w * 0.5f * dsc(1), h * 0.5f * (1 + dsc(3))) {
        rotate(rot * dsc(2))
...
fun Canvas.drawSCRDNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawSemiCircleRotDown(scale, w, h, paint)
}

class SemiCircleRotDownView(ctx : Context) : View(ctx) {
    ...
    data class SemiCircleRotDown(var i : Int) {
