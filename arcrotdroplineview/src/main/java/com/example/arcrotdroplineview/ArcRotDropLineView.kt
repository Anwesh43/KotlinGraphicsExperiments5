package com.example.arcrotdroplineview
...
class ArcRotDropLineView(ctx : Context) : View(ctx) {
    ...
    data class ARDLNode(var i : Int = 0, val state : State = State()) {

        private var next : ARDLNode? = null
        private var prev : ARDLNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawARDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : ARDLNode {
            var curr : ARDLNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ArcRotDropLine(var i : Int) {

        private var curr : ARDLNode = ARDLNode(0)
