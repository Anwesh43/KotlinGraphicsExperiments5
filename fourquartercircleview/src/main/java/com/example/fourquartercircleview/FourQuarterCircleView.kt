package com.example.fourquartercircleview
...
class FourQuarterCircleView(ctx : Context) : View(ctx) {
    ...
    data class FQCNode(var i : Int = 0, val state : State = State()) {

        private var next : FQCNode? = null
        private var prev : FQCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawFQCNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : FQCNode {
            var curr : FQCNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class FourQuarterCircle(var i : Int) {

        private var curr : FQCNode = FQCNode(0)
