package com.example.bilinequarterarcview
...
fun Canvas.drawBiLineQuarterArc(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2) * dsc(3)) {
        ...
        for (j in 0..1) {
            drawXY(size * 0.5f * j, 0f) {
                drawLine(0f, 0f, 0f, -size * 0.5f * dsc(j).divideScale(0, 1), paint)
                drawArc(RectF(-size / 2, -size / 2, size / 2, size / 2), -90f, 90f * dsc(j).divideScale(1, 2), false, paint)
            }
        }
    }
}
...
fun Canvas.drawBLQANode(i : Int, scale : Float, paint : Paint) {
    ...
    drawBiLineQuarterArc(scale, w, h, paint)
}

class BiLineQuarterArcView(ctx : Context) : View(ctx) {
    ...
    data class Renderer(var view : BiLineQuarterArcView) {

        private val animator : Animator = Animator(view)
        private val blqa : BiLineQuarterArc = BiLineQuarterArc(0)
