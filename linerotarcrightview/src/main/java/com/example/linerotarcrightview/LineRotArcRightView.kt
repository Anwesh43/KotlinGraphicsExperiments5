...
class LineRotArcRightView(ctx : Context) : View(ctx) {
    ...
    data class LRARNode(var i : Int = 0, val state : State = State()) {
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LRARNode {
    ...
    data class LineRotArcRight(var i : Int) {

        private var curr : LRARNode = LRARNode(0)
    ...
    data class Renderer(var view : LineRotArcRightView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val lrar : LineRotArcRight = LineRotArcRight(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            lrar.draw(canvas, paint)
            animator.animate {
                lrar.update {
                    animator.stop()
                }
    ...
    companion object {
        ...
        fun create(activity: Activity) : LineRotArcRightView {
            val view : LineRotArcRightView = LineRotArcRightView(activity)
