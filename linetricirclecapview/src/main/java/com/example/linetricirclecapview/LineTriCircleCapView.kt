...
class LineTriCircleCapView(ctx : Context) : View(ctx) {
    ...
    data class LTCCNode(var i : Int = 0, val state : State = State()) {

        private var next : LTCCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLTCCNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LTCCNode {
            var curr : LTCCNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineTriCircleCap(var i : Int) {

        private var curr : LTCCNode = LTCCNode(0)
    ...
    data class Renderer(var view : LineTriCircleCapView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val ltcc : LineTriCircleCap = LineTriCircleCap(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            ltcc.draw(canvas, paint)
            animator.animate {
                ltcc.update {
                    animator.stop()
                }
