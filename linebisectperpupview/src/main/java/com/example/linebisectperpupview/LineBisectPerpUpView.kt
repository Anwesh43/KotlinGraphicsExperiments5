package com.example.linebisectperpupview
...
class LineBisectPerpUpView(ctx : Context) : View(ctx) {
    ...
    data class LBPUNode(var i : Int = 0, val state : State = State()) {

        private var next : LBPUNode? = null
        private var prev : LBPUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLBPUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LBPUNode {
            var curr : LBPUNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineBisectPerpUp(var i : Int) {

        private var curr : LBPUNode = LBPUNode(0)
