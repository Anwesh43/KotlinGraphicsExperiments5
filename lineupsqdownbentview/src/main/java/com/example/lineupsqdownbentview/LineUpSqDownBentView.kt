package com.example.lineupsqdownbentview
...
fun Canvas.drawLineUpSqDownBent(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(3), h / 2) {
        drawXY(0f, -h * 0.5f * (1 - dsc(0))) {
            drawRect(RectF(0f, -size / 2, size / 2, 0f), paint)
        }
...
fun Canvas.drawLUSDBNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = Color.parseColor(colors[i])
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineUpSqDownBent(scale, w, h, paint)
}

class LineUpSqDownBentView(ctx : Context) : View(ctx) {
    ...
    data class LineUpSqDownBent(var i : Int) {

        private var curr : LUSDBNode = LUSDBNode(0)
